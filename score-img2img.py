...
def init():
    ...
    model_path = os.path.join(os.getenv("AZUREML_MODEL_DIR"), "stable-diffusion-v1-4.zip")

    with ZipFile(model_path, 'r') as zipObj:
        # Extract all the contents of zip file in a directory
        zipObj.extractall('stable-difussion')

    model_id = "stable-difussion/stable-diffusion-v1-4"
    device = "cuda"

    global inference_pipeline

    inference_pipeline = StableDiffusionImg2ImgPipeline.from_pretrained(model_id, use_auth_token=True,)
